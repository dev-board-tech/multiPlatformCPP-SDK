...
SysErr GI::Dev::DevRequest::request(char *path, GI::Display::Display **device)
{
	unsigned int dev_nr = 0;
	while(1)
	{
	...
	*device = (GI::Display::Display *)ioSetCfg[dev_nr]->ioConstruct;
	return SYS_ERR_OK;
}

SysErr GI::Dev::DevRequest::request(char *path, GI::Display::Cursor **device)
{
	unsigned int dev_nr = 0;
	while(1)
	{
	...
	*device = (GI::Display::Cursor *)ioSetCfg[dev_nr]->ioConstruct;
	return SYS_ERR_OK;
}
