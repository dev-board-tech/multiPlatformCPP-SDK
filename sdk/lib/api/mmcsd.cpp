...
SysErr GI::Storage::MmcSd::ioctl(unsigned int command,
		unsigned int *buffer)
{
    if(ioctl_Ptr)
...
SysErr GI::Storage::MmcSd::read(void *ptr,
...
SysErr GI::Storage::MmcSd::write(void *ptr,
		unsigned long block, unsigned int nblks)
{
    if(write_Ptr)
...
SysErr GI::Storage::MmcSd::getRawCid(unsigned long *rawCid)
{
	if(getRawCid_Ptr)
	{
    	if(!rawCid)
...
SysErr GI::Storage::MmcSd::getRawCsd(unsigned long *rawCsd)
{
    if(getRawCsd_Ptr)
    {
    	if(!rawCsd)
