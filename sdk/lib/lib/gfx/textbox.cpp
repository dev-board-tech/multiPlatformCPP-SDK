...
void GI::Screen::Gfx::TextBox::paint(void *pDisplay, s32 x_start, s32 y_start, s32 x_len, s32 y_len, tControlCommandData* control_comand)
{
	...
	u32 color = Color.Enabled.Buton.Pull;
	if(!Enabled || !(parentWindowHandler ? parentWindowHandler->Internals.OldStateEnabled : true))
		color = Color.Disabled.Buton;
	if(Internals.NeedEntireRefresh)
	{
		if((!Enabled || !(parentWindowHandler ? parentWindowHandler->Internals.OldStateEnabled : true))/* && Internals.Control.Initiated == true*/) {
...
void GI::Screen::Gfx::TextBox::idle(tControlCommandData* control_comand)
{
	...
	if(Internals.Control.Initiated == false)
	{
		...
		else
		{
			...
			Internals.Vscrollbar = new GI::Screen::Gfx::ScrollBar(Internals.pDisplay);
			Internals.Hscrollbar = new GI::Screen::Gfx::ScrollBar(Internals.pDisplay);
		}
					...
					Visible == true)
	{
		...
		{
			Internals.Vscrollbar->Internals.PositionOffset.X = Internals.Position.X;
			Internals.Vscrollbar->Internals.PositionOffset.Y = Internals.Position.Y;
		}
		...
		{
			Internals.Hscrollbar->Internals.PositionOffset.X = Internals.Position.X;
			Internals.Hscrollbar->Internals.PositionOffset.Y = Internals.Position.Y;
		}
		...
		//settings->Internals.Caption.Text = change_str(settings->Internals.Caption.Text, settings->Text);
...
GI::Screen::Gfx::TextBox::TextBox(void *parentWindowHandler)
{
	...
	GI::Screen::Gfx::String *tmp_text = new GI::Screen::Gfx::String(Internals.pDisplay);
...
GI::Screen::Gfx::TextBox::TextBox(GI::Display::Display *ScreenDisplay)
{
    if(!this)
		return;
    if(!ScreenDisplay)
...
//#######################################################################################
bool GI::Screen::Gfx::TextBox::insert(char* string, u32 location)
{
	if(!this)
		return false;
