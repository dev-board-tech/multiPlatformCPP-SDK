...
void GI::Screen::Gfx::ScrollBar::paint(void *pDisplay, s32 x_start, s32 y_start, s32 x_len, s32 y_len, tControlCommandData* control_comand)
{
	...
	s32 ValueBetweenMinAndMax = (Internals.OldMaximum - Internals.OldMinimum);
	...
	if(Size.X < Size.Y)
	{
		...
		{
			BtnUpSettings->Internals.PositionOffset.X = x_start;
			BtnUpSettings->Internals.PositionOffset.Y = y_start;
		}
		...
		{
			BtnDnSettings->Internals.PositionOffset.X = x_start;
			BtnDnSettings->Internals.PositionOffset.Y = y_start;
		}
	...
	else
	{
					...
					Visible == true)
		{
			if(Size.X < Size.Y)
			{
				...
				////s32 Steps = settings->Size.Y - 4 - BtnSize;
				//ButtonSettings->Position.X = settings->Position.X + 2;
				////ButtonSettings->Position.Y = settings->Position.Y + 2;
				ButtonSettings->Size.X = Size.X - 4;
				...
				{
					ButtonSettings->Position.Y = Internals.CoordonateOfButtonDown + ((control_comand->Y - (parentWindowHandler ? parentWindowHandler->Internals.Position.Y : 0)) - Internals.CoordonateOfTouchDown);
					...
					Value = (s32)GUtil::Converters::percentTo((s32)Minimum, (s32)Maximum, (s32)(Size.Y - 4 - BtnSize - ((Size.X - 2)<<1)), (s32)(ButtonSettings->Position.Y - (Position.Y + 2) - (Size.X - 2)));
			...
			else
			{
				...
				////s32 Steps = settings->Size.X - 4 - BtnSize;
				//ButtonSettings->Position.Y = settings->Position.Y + 2;
				////ButtonSettings->Position.X = settings->Position.X + 2;
				ButtonSettings->Size.Y = Size.Y - 4;
				...
				{
					ButtonSettings->Position.X = Internals.CoordonateOfButtonDown + ((control_comand->X - (parentWindowHandler ? parentWindowHandler->Internals.Position.X : 0)) - Internals.CoordonateOfTouchDown);
					...
					Value = (s32)GUtil::Converters::percentTo((s32)Minimum, (s32)Maximum, (s32)(Size.X - 4 - BtnSize - ((Size.Y - 2)<<1)), (s32)(ButtonSettings->Position.X - (Position.X + 2) - (Size.Y - 2)));
	...
	else if(ButtonSettings->Position.X + parentWindowHandler ? parentWindowHandler->Internals.Position.X : 0 + ButtonSettings->Internals.PositionOffset.X != ButtonSettings->Internals.Position.X ||
			ButtonSettings->Position.Y + parentWindowHandler ? parentWindowHandler->Internals.Position.Y : 0 + ButtonSettings->Internals.PositionOffset.Y != ButtonSettings->Internals.Position.Y)
	{
		ButtonSettings->Internals.NeedEntireRefresh = true;
		CursorState back = control_comand->Cursor;
	...
	else
	{
		...
		else if(ValueIsChangedExternaly)
		{
			ButtonSettings->Internals.NeedEntireRefresh = true;
			control_comand->Cursor = Cursor_Up;
		}
		else if(BtnUpSettings->Internals.CursorDownInsideBox || BtnDnSettings->Internals.CursorDownInsideBox)
		{
			control_comand->Cursor = Cursor_Up;
		}
		ButtonSettings->idle(control_comand);
...
void GI::Screen::Gfx::ScrollBar::idle(tControlCommandData* control_comand)
{
	...
	if(Internals.Control.Initiated == false)
	{
		...
		{
			Internals.Position.X = Position.X + Internals.PositionOffset.X;
			Internals.Position.Y = Position.Y + Internals.PositionOffset.Y;
		}
		...
		if(parentWindowHandler)
			...
			Internals.BtnSettings = new GI::Screen::Gfx::Button(Internals.pDisplay);
		GI::Screen::Gfx::Button* ButtonSettings = Internals.BtnSettings;
		ButtonSettings->Internals.NoPaintBackGround = false;
		...
		if(parentWindowHandler)
			...
			Internals.BtnUpSettings = new GI::Screen::Gfx::Button(Internals.pDisplay);
		GI::Screen::Gfx::Button* BtnUpSettings = Internals.BtnUpSettings;
		...
		if(parentWindowHandler)
			...
			Internals.BtnDnSettings = new GI::Screen::Gfx::Button(Internals.pDisplay);
		GI::Screen::Gfx::Button* BtnDnSettings = Internals.BtnDnSettings;
		...
		if(Size.X < Size.Y)
		{
			...
			if(parentWindowHandler)
			{
				ButtonSettings->Internals.PositionOffset.Y = Internals.Position.Y - parentWindowHandler->Internals.Position.Y;
				ButtonSettings->Internals.PositionOffset.X = Internals.Position.X - parentWindowHandler->Internals.Position.X;
			}
			else
			{
				ButtonSettings->Internals.PositionOffset.Y = Internals.Position.Y;
				ButtonSettings->Internals.PositionOffset.X = Internals.Position.X;
			}
			Internals.BtnSettings->Size.X = Internals.Size.X - 4;
			//ButtonSettings->Size.Y = settings->Internals.Size.X - 4;
		...
		else
		{
			...
			//ButtonSettings->Size.X = settings->Internals.Size.Y - 4;
			ButtonSettings->Size.Y = Internals.Size.Y - 4;
			/* Left */
			BtnUpSettings->Position.X = 2;
			BtnUpSettings->Position.Y = 2;
			BtnUpSettings->Internals.PositionOffset.X = Internals.Position.X;
	...
	{
		if((Position.X + Internals.PositionOffset.X != Internals.Position.X ||
			Position.Y + Internals.PositionOffset.Y != Internals.Position.Y ||
				Size.X != Internals.Size.X ||
					Size.Y != Internals.Size.Y ||
						Size.MinBtnSize != Internals.Size.MinBtnSize ||
							Minimum != Internals.OldMinimum||
					...
					Visible == true)
	{
		...
		if(Size.X < Size.Y)
		{
			...
			{
				Internals.BtnSettings->Internals.PositionOffset.Y = Internals.Position.Y;
				Internals.BtnSettings->Internals.PositionOffset.X = Internals.Position.X;
			}
		}
		else
		{
...
GI::Screen::Gfx::ScrollBar::ScrollBar(GI::Display::Display *ScreenDisplay)
{
    if(!this)
		return;
    if(!ScreenDisplay)
