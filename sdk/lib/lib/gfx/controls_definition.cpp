...
u32 controlls_change_color(u32 color, double value)
{
	if(value>(double)0)
	{
		...
		return (u32)((u32)Red|(((u32)Green)<<8)|(((u32)Blue)<<16));
	}
...
u32 controlls_change_color(u32 color, double value)
{
	if(value>(double)0)
	{
		...
		return (u32)((u32)Red|(((u32)Green)<<5)|(((u32)Blue)<<11));
	}
	else if(value<(double)0)
	{
		double Red   = (double)((color)    &0x1F);
...
#endif
//#######################################################################################
//bool screen_copy(tDisplay *pDisplayTo, tDisplay *pDisplayFrom, bool put_cursor, s32 X, s32 Y, u32 color)
//{
	...
	//memcpy((void *)pDisplayTo->DisplayData, (void *)pDisplayFrom->DisplayData, (sizeof(u32) * pDisplayFrom->Height * pDisplayFrom->Width) + 32);
	//box_cache_clean(pDisplayTo, 0, 0, pDisplayFrom->Width, pDisplayFrom->Height);
	s32 LineCnt = 0;
	volatile u32* ScreenBuff = pDisplayTo->DisplayData + pDisplayTo->raster_timings->palete_len;
	volatile u32* _ScreenBuff = pDisplayFrom->DisplayData + pDisplayTo->raster_timings->palete_len;
	for(; LineCnt < pDisplayTo->raster_timings->Y; LineCnt ++)
	{
		...
		CacheDataCleanInvalidateBuff((u32)((u32*)(ScreenBuff + (pDisplayFrom->raster_timings->X * LineCnt))), (sizeof(ScreenBuff[0]) * pDisplayTo->raster_timings->X) + 64);
	}
	return true;*/
