...
void GI::Screen::Gfx::Picturebox::paint(void *pDisplay, s32 x_start, s32 y_start, s32 x_len, s32 y_len, tControlCommandData* control_comand)
{
	u32 color = 0;
	...
	if(Events.Refresh.CallBack)
	{
		...
		s32 X = control_comand->X;
		s32 Y = control_comand->Y;
...
void GI::Screen::Gfx::Picturebox::idle(tControlCommandData* control_comand)
{
	...
	if(control_comand->Comand != Control_Nop)
	{
		...
		switch((u8)control_comand->Comand)
#else
		switch((u32)control_comand->Comand)
#endif
		{
	...
	s32 X_StartBox = Internals.Position.X;
	s32 Y_StartBox = Internals.Position.Y;
	s32 X_LenBox = Internals.Size.X;
	s32 Y_LenBox = Internals.Size.Y;
...
GI::Screen::Gfx::Picturebox::Picturebox(GI::Display::Display *ScreenDisplay)
{
    if(!this)
		return;
    if(!ScreenDisplay)
        return;
    memset(this, 0, sizeof(*this));
    Internals.parentWindowHandler = NULL;

    Internals.pDisplay = ScreenDisplay;
...
void GI::Screen::Gfx::Picturebox::clear()
{
	if(!this)
		return;
	GI::Screen::Gfx::Window *parentWindowHandler = (GI::Screen::Gfx::Window*)Internals.parentWindowHandler;
	GI::Display::Display* LcdStruct = Internals.pDisplay;
	tRectangle back_up_clip = LcdStruct->sClipRegion;
	s32 X_StartBox = Internals.Position.X;
	s32 Y_StartBox = Internals.Position.Y;
	s32 X_LenBox = Internals.Size.X;
...
/*//#######################################################################################
void picturebox_copy_rectangle(tPictureBox* settings, u32 *src_buff, s32 src_x_buff_size, s32 src_y_buff_size, s32 src_x_offset, s32 src_y_offset)
{
	...
	s32 Y_cnt = 0;
	s32 X_len = (pDisplay->sClipRegion.sXMax - pDisplay->sClipRegion.sXMin);
	if(X_len <= 0) return;
	for(; Y_cnt < src_y_offset + (pDisplay->sClipRegion.sYMax - pDisplay->sClipRegion.sYMin); Y_cnt++)
	{
		...
		CacheDataCleanInvalidateBuff((u32)(void *)(pDisplay->DisplayData + 8 + ((Y_cnt + settings->Position.Y + 2) * pDisplay->Width) + settings->Position.X + 2), X_len * sizeof(pDisplay->DisplayData[0]) + 64);
	}
	pDisplay->sClipRegion = back_up_clip;
}*/
//#######################################################################################
void GI::Screen::Gfx::Picturebox::copyRectangle(u32 *src_buff, u32 src_buff_data_offset, tRectangle *_dest_rectangle, tRectangle *_src_rectangle, s32 src_width, s32 src_height)
{
	if(!this)
		return;
	//tWindow *ParentWindow = (tWindow*)settings->Internals.ParentWindow;
	GI::Display::Display* pDisplay = Internals.pDisplay;
	//tRectangle back_up_clip = pDisplay->sClipRegion;
	//pDisplay->sClipRegion = settings->Internals.PictureWindowLimits;
	////put_pixel(pDisplay, X + settings->Position.X + 2, Y + settings->Position.Y + 2, color);
	...
	s32 dest_X_StartBox = Internals.Position.X;
	s32 dest_Y_StartBox = Internals.Position.Y;
	//s32 dest_X_EndBox = settings->Internals.Position.X + settings->Internals.Size.X;
	//s32 dest_Y_EndBox = settings->Internals.Position.Y + settings->Internals.Size.Y;
	...
	if(dest_rectangle->sXMax < Internals.Position.X + 2 ||
		...
		return;
	pDisplay->copyRectangle32Bit((u8 *)src_buff, dest_rectangle->sXMin, dest_rectangle->sYMin, dest_rectangle->sXMax - dest_rectangle->sXMin, dest_rectangle->sYMax - dest_rectangle->sYMin);
	...
	s32 x_line_len = dest_rectangle->sXMax - dest_rectangle->sXMin;

	if(x_line_len <= 0) return;
	if(dest_X_StartBox <= dest_rectangle->sXMin)
		dest_X_StartBox = dest_rectangle->sXMin;

	s32 Y_cnt = dest_Y_StartBox;
	if(Y_cnt <= dest_rectangle->sYMin)
		Y_cnt = dest_rectangle->sYMin;

	s32 X_Start_Src_Buff = (settings->Internals.Position.X + 2) - settings->Internals.PictureWindowLimits.sXMin;
	if(X_Start_Src_Buff > 0)
		X_Start_Src_Buff = 0;
	else
		X_Start_Src_Buff = (~X_Start_Src_Buff) + 1;
	s32 Y_Start_Src_Buff = (settings->Internals.Position.Y + 2) - settings->Internals.PictureWindowLimits.sYMin;
...
//#######################################################################################
void GI::Screen::Gfx::Picturebox::drawPixel(s32 X, s32 Y, u32 color)
{
	if(!this)
		return;
	//tWindow *ParentWindow = (tWindow*)settings->Internals.ParentWindow;
...
//#######################################################################################
void GI::Screen::Gfx::Picturebox::drawHLine(s32 X1, s32 X2, s32 Y, u8 width, u32 color)
{
	if(!this)
		return;
	//tWindow *ParentWindow = (tWindow*)settings->Internals.ParentWindow;
...
//#######################################################################################
void GI::Screen::Gfx::Picturebox::drawVLine(s32 Y1, s32 Y2, s32 X, u8 width, u32 color)
{
	if(!this)
		return;
	//tWindow *ParentWindow = (tWindow*)settings->Internals.ParentWindow;
...
//#######################################################################################
void GI::Screen::Gfx::Picturebox::drawCircle(s32 x, s32 y, s32 radius, u8 fill, u32 color)
{
	if(!this)
		return;
	//tWindow *ParentWindow = (tWindow*)settings->Internals.ParentWindow;
...
//#######################################################################################
void GI::Screen::Gfx::Picturebox::drawLine(s32 X1, s32 Y1, s32 X2, s32 Y2, u8 width, u32 color)
{
	if(!this)
		return;
	//tWindow *ParentWindow = (tWindow*)settings->Internals.ParentWindow;
...
//#######################################################################################
void GI::Screen::Gfx::Picturebox::drawElipse(s32 xc,s32 yc,s32 rx,s32 ry, u8 fill, u32 color)
{
	if(!this)
		return;
	//tWindow *ParentWindow = (tWindow*)settings->Internals.ParentWindow;
...
//#######################################################################################
void GI::Screen::Gfx::Picturebox::drawTriangle(s32  Ax,s32  Ay,s32  Bx,s32  By,s32  Cx,s32  Cy, u8 fill, u32 color)
{
	if(!this)
		return;
	//tWindow *ParentWindow = (tWindow*)settings->Internals.ParentWindow;
...
//#######################################################################################
void GI::Screen::Gfx::Picturebox::drawString(GI::Screen::Gfx::String *string)
{
	if(!this)
		return;
	//tWindow *ParentWindow = (tWindow*)settings->Internals.ParentWindow;
...
//#######################################################################################
void GI::Screen::Gfx::Picturebox::draw3dTriangle(_3d_points *Points, s32 X_offset, s32 Y_offset, double X_Angle, double Y_Angle, double Z_Angle, u32 Color)
{
	if(!this)
		return;
	//tWindow *ParentWindow = (tWindow*)settings->Internals.ParentWindow;
...
//#######################################################################################
void GI::Screen::Gfx::Picturebox::draw3dCube(_3d_points *Points, s32 X_offset, s32 Y_offset, double X_Angle, double Y_Angle, double Z_Angle, u32 Color)
{
	if(!this)
		return;
	//tWindow *ParentWindow = (tWindow*)settings->Internals.ParentWindow;
	...
	Points->length = 8;
	Transform3DPointsTo2DPoints(&screenPoints, Points, cubeAxisRotations);
	drawLine((s32)screenPoints.x[0] + X_offset, (s32)screenPoints.y[0] + Y_offset, (s32)screenPoints.x[1] + X_offset, (s32)screenPoints.y[1] + Y_offset, 1, Color);
	drawLine((s32)screenPoints.x[0] + X_offset, (s32)screenPoints.y[0] + Y_offset, (s32)screenPoints.x[3] + X_offset, (s32)screenPoints.y[3] + Y_offset, 1, Color);
	drawLine((s32)screenPoints.x[0] + X_offset, (s32)screenPoints.y[0] + Y_offset, (s32)screenPoints.x[4] + X_offset, (s32)screenPoints.y[4] + Y_offset, 1, Color);

	drawLine((s32)screenPoints.x[2] + X_offset, (s32)screenPoints.y[2] + Y_offset, (s32)screenPoints.x[1] + X_offset, (s32)screenPoints.y[1] + Y_offset, 1, Color);
	drawLine((s32)screenPoints.x[2] + X_offset, (s32)screenPoints.y[2] + Y_offset, (s32)screenPoints.x[3] + X_offset, (s32)screenPoints.y[3] + Y_offset, 1, Color);
	drawLine((s32)screenPoints.x[2] + X_offset, (s32)screenPoints.y[2] + Y_offset, (s32)screenPoints.x[6] + X_offset, (s32)screenPoints.y[6] + Y_offset, 1, Color);

	drawLine((s32)screenPoints.x[5] + X_offset, (s32)screenPoints.y[5] + Y_offset, (s32)screenPoints.x[1] + X_offset, (s32)screenPoints.y[1] + Y_offset, 1, Color);
	drawLine((s32)screenPoints.x[5] + X_offset, (s32)screenPoints.y[5] + Y_offset, (s32)screenPoints.x[4] + X_offset, (s32)screenPoints.y[4] + Y_offset, 1, Color);
	drawLine((s32)screenPoints.x[5] + X_offset, (s32)screenPoints.y[5] + Y_offset, (s32)screenPoints.x[6] + X_offset, (s32)screenPoints.y[6] + Y_offset, 1, Color);

	drawLine((s32)screenPoints.x[7] + X_offset, (s32)screenPoints.y[7] + Y_offset, (s32)screenPoints.x[3] + X_offset, (s32)screenPoints.y[3] + Y_offset, 1, Color);
	drawLine((s32)screenPoints.x[7] + X_offset, (s32)screenPoints.y[7] + Y_offset, (s32)screenPoints.x[4] + X_offset, (s32)screenPoints.y[4] + Y_offset, 1, Color);
	drawLine((s32)screenPoints.x[7] + X_offset, (s32)screenPoints.y[7] + Y_offset, (s32)screenPoints.x[6] + X_offset, (s32)screenPoints.y[6] + Y_offset, 1, Color);
}
//#######################################################################################
#if _USE_BITMAP_LIBRARY == 1
