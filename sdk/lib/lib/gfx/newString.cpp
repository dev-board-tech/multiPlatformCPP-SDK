...
/*#####################################################*/
#ifdef __AVR_XMEGA__
const u8 newCharTable6x8[] PROGMEM =
#else
const u8 newCharTable6x8[] =
#endif
{
	6                          ,0          ,6          ,8          ,32            ,128,
...
GI::Gfx::string::string() :
				buff(NULL), pFont((tFont*) systemFont), length(0), opaque(true), visible(true), wordWrap(true), x(0), y(0), selStart(0), selLen(0), textAlign(GI::textAllign::center), isModifyed(true), error(SYS_ERR_BUFF_NOT_ALLOCATED), terminalMode(false)
{
	foreColor.ARGB = 0;
	backColor.ARGB = 0;
	this->pDisplay = dev.SCREEN[0];
	this->window.sXMin = 0;
	this->window.sXMax = this->pDisplay->LcdTimings->X;
	this->window.sYMin = 0;
	this->window.sYMax = this->pDisplay->LcdTimings->Y;
...
GI::Gfx::string::string(char *string) :
		buff(NULL), pFont((tFont*) systemFont), length(0), opaque(true), visible(true), wordWrap(true), x(0), y(0), selStart(0), selLen(0), textAlign(GI::textAllign::center), isModifyed(true), error(SYS_ERR_BUFF_NOT_ALLOCATED), terminalMode(false)
{
	foreColor.ARGB = 0;
	backColor.ARGB = 0;
...
GI::Gfx::string::string(GI::Gfx::string *string, u32 position, u32 len) :
		buff(NULL), pFont((tFont*) systemFont), length(0), opaque(true), visible(true), wordWrap(true), x(0), y(0), selStart(0), selLen(0), textAlign(GI::textAllign::center), isModifyed(true), error(SYS_ERR_BUFF_NOT_ALLOCATED), terminalMode(false)
{
	foreColor.ARGB = 0;
	backColor.ARGB = 0;
...
GI::Gfx::string::string(GI::string *string, u32 position, u32 len) :
		buff(NULL), pFont((tFont*) systemFont), length(0), opaque(true), visible(true), wordWrap(true), x(0), y(0), selStart(0), selLen(0), textAlign(GI::textAllign::center), isModifyed(true), error(SYS_ERR_BUFF_NOT_ALLOCATED), terminalMode(false)
{
	foreColor.ARGB = 0;
	backColor.ARGB = 0;
...
GI::Gfx::string::string(char *string, u32 position, u32 len) :
		buff(NULL), pFont((tFont*) systemFont), length(0), opaque(true), visible(true), wordWrap(true), x(0), y(0), selStart(0), selLen(0), textAlign(GI::textAllign::center), isModifyed(true), error(SYS_ERR_BUFF_NOT_ALLOCATED), terminalMode(false)
{
	foreColor.ARGB = 0;
	backColor.ARGB = 0;
...
bool GI::Gfx::string::equal(GI::Gfx::string *string)
{
	if(!this)
		return false;
	if(!string)
	...
	if((length != string->length) ||
		(strcmp(buff, string->buff) == 0))
		return false;
	return true;
}

bool GI::Gfx::string::equal(GI::string *string)
{
	if(!this)
		return false;
	if(!string)
...
bool GI::Gfx::string::equal(char *string)
{
	if(!this)
		return false;
	if(!string)
...
void GI::Gfx::string::removeNewLine(GI::Gfx::string *string)
{
	if(!this)
		return;
	if(!string)
...
void GI::Gfx::string::setText(char* string)
{
	if(!this)
		return;
	if(!string)
	...
	if (!Return)
	{
		error = SYS_ERR_BUFF_NOT_ALLOCATED;
		return;
	}
	strcpy(Return, string);
	buff = Return;
	length = LenSrc;
	isModifyed = true;
	error = SYS_ERR_OK;
...
void GI::Gfx::string::setText(GI::Gfx::string *string)
{
	if(!this)
		return;
	if(!string)
...
void GI::Gfx::string::setText(GI::string *string)
{
	if(!this)
		return;
	if(!string)
...
void GI::Gfx::string::append(char* string)
{
	if(!this)
		return;
	if(!string)
...
void GI::Gfx::string::append(GI::Gfx::string *string)
{
	if(!this)
		return;
	if(!string)
...
void GI::Gfx::string::append(GI::string *string)
{
	if(!this)
		return;
	if(!string)
...
void GI::Gfx::string::append(char Char)
{
	if(!this)
		return;
	if(!buff)
		return;
	u32 LenDest = length;
	char *Return = (char *) realloc(buff, LenDest + 2);
	//char *Return = (char *)realloc(dest, LenDest + LenSrc + 1);
	if (!Return)
	{
		error = SYS_ERR_BUFF_NOT_ALLOCATED;
...
void GI::Gfx::string::insert(GI::Gfx::string* string, u32 location)
{
	if(!this)
		return;
	if(!string)
	...
	if (!Return)
	{
		string->error = SYS_ERR_BUFF_NOT_ALLOCATED;
		return;
	}
...
void GI::Gfx::string::insert(GI::string* string, u32 location)
{
	if(!this)
		return;
	if(!string)
...
void GI::Gfx::string::insert(char *string, u32 location)
{
	if(!this)
		return;
	if(!string)
...
void GI::Gfx::string::clone(GI::Gfx::string* string)
{
	if(!this)
		return;
	if (!string)
    ...
    foreColor.ARGB = string->foreColor.ARGB;
    opaque = string->opaque;
	visible = string->visible;
	wordWrap = string->wordWrap;
	x = string->x;
	y = string->y;
	selStart = string->selStart;
	selLen = string->selLen;
	textAlign = string->textAlign;
	pDisplay = string->pDisplay;
}

void GI::Gfx::string::clear()
{
	if(!this)
		return;
	if(!buff)
...
void GI::Gfx::string::subString(GI::Gfx::string *string, u32 position, u32 len)
{
	if(!this || !string)
		return;
	if(!string->buff)
		return;
	if (position + len >= (u32) string->length)
		return;
	...
	strncpy(Return, string->buff + position, len);
	setText(Return);
	free(Return);
	error = SYS_ERR_OK;
}

void GI::Gfx::string::subString(GI::string *string, u32 position, u32 len)
{
	if(!this || !string)
		return;
	if(!string->buff)
...
void GI::Gfx::string::subString(char *string, u32 position, u32 len)
{
	if(!this)
		return;
	if(!string)
		return;
	if (position + len >= (u32) strlen(string))
		return;
	char *Return = (char *) calloc(1, len + 1);
	if (!Return)
	{
...
void GI::Gfx::string::getStrSelect(
		...
		u32 *return_command)
{
	...
	char *pcString = buff;
	s32 lLength = -1;
	if (window.sXMin > _XPull)
	{
		*return_command = ReturnCommand_GoLeft;
		u32 tmp = window.sXMin - _XPull;
		if (tmp > ReturnCommand_MaxValue)
			tmp = ReturnCommand_MaxValue;
		*return_command |= tmp;
	}
	if (window.sXMax < _XPull)
	{
		*return_command = ReturnCommand_GoRight;
		u32 tmp = _XPull - window.sXMax;
		if (tmp > ReturnCommand_MaxValue)
			tmp = ReturnCommand_MaxValue;
		*return_command |= tmp;
	}
	if (window.sYMin > _YPull)
	{
		*return_command = ReturnCommand_GoUp;
		u32 tmp = window.sYMin - _YPull;
		if (tmp > ReturnCommand_MaxValue)
			tmp = ReturnCommand_MaxValue;
		*return_command |= tmp;
	}
	if (window.sYMax < _YPull)
	{
		*return_command = ReturnCommand_GoDn;
		u32 tmp = _YPull - window.sYMax;
		if (tmp > ReturnCommand_MaxValue)
			tmp = ReturnCommand_MaxValue;
		*return_command |= tmp;
	}
	...
	for (; *pcString && lLength; pcString++, lLength--)
	{
		...
		// with a ".".
		//
		if (((StringLengthOfEveryRow - window.sXMin) + read_data_byte(pucData[read_data_word(pusOffset[*pcString - ' '] ) + 1])
				>= (window.sXMax - window.sXMin) && wordWrap == true))
		{
			//pucData = 0;
...
u32 GI::Gfx::string::getStrRowsNr()
{
	if(!this)
		return 0;
	char *pcString = buff;
...
StringProperties_t GI::Gfx::string::getStrProp()
{
	...
	for (; *pcString && lLength; pcString++, lLength--)
	{
		...
		// with a ".".
		//
		if ((StringLengthOfEveryRow + read_data_byte(pucData[read_data_word(pusOffset[*pcString - ' '] ) + 1]) >= (window.sXMax - window.sXMin) &&
				wordWrap == true))
		{
			//pucData = 0;
...
s32 GI::Gfx::string::drawString()
{
	if(!this)
		return 0;
	if (!pDisplay || !pFont || !buff)
		return 0;
	char *pcString = buff;
	int lLength = -1;
	...
	while (*pcString && lLength--)
	{
		...
		{
			y += read_data_byte(pFont->ucHeight);
			pucData = 0;
			pcString++;
		}
		else
		{
			...
			if (x + read_data_byte(pucData[1]) >= window.sXMax && wordWrap == true)
			{
				x = lXBackup;
				y += read_data_byte(pFont->ucHeight);
			}
			...
			//
			if ((x + read_data_byte(pucData[1])) < window.sXMin)
			{
				//
				// Increment the X coordinate by the width of the character.
				//
				x += read_data_byte(pucData[1]);
			...
			for (lIdx = 2, lX0 = 0, lBit = 0, lY0 = 0; lIdx < read_data_byte(pucData[0]);)
			{
				...
				while (lOff)
				{
					...
					if ((lOff > 1) && ((lX0 + 1) < read_data_byte(pucData[1])))
					{
						...
						// pixels.
						//
						if(((y + lY0) >= window.sYMin) && _opaque)
						{
							if(visible == true)
								pDisplay->drawHLine(x + lX0, lCount + 1, y + lY0, 1, backColor.ARGB);
						}
					...
					else
					{
						...
						// it.
						//
						if(((x + lX0) >= window.sXMin) &&
							((x + lX0) <= window.sXMax) &&
							((y + lY0) >= window.sYMin) && _opaque == true)
						{
#if (__SIZEOF_INT__ == 2)
							if(visible == true)
								pDisplay->drawPixel(x + lX0, y + lY0, backColor.ARGB);
#else
							if(visible == true)
								pDisplay->drawPixel(x + lX0, y + lY0, backColor.ARGB<<8);
#endif
						}
				...
				while (lOn)
				{
					...
					if ((lOn > 1) && ((lX0 + 1) < read_data_byte(pucData[1])))
					{
						...
						// pixels.
						//
						if((y + lY0) >= window.sYMin)
						{
							if(visible == true)
								pDisplay->drawHLine(x + lX0, lCount + 1, y + lY0, 1, foreColor.ARGB);
						}
...
#if (__SIZEOF_INT__ == 2)
							if(visible == true)
								pDisplay->drawPixel(x + lX0, y + lY0, foreColor.ARGB);
#else
							if(visible == true)
								pDisplay->drawPixel(x + lX0, y + lY0, foreColor.ARGB<<8);
#endif
						}
...
s32 GI::Gfx::string::drawStringTiny()
{
	if(!this)
		return 0;
	GI::Display::Display* LcdStruct = (GI::Display::Display *) pDisplay;
	...
	s32 CharCnt = 0;
#ifdef __AVR_XMEGA__
	chWidth = pgm_read_byte(&newCharTable6x8[2]);
	chHeight = pgm_read_byte(&newCharTable6x8[3]);
#else
	chWidth = newCharTable6x8[2];
	chHeight = newCharTable6x8[3];
#endif
	do
	{
		u8 Char = *pcString;
		if (Char == 0)
			return CharCnt - 1;
#ifdef __AVR_XMEGA__
		CharPtr = ((Char - pgm_read_byte(&newCharTable6x8[4])) * chWidth) + pgm_read_byte(&newCharTable6x8[0]);
		if(Char < pgm_read_byte(&newCharTable6x8[4]) || Char > pgm_read_byte(&newCharTable6x8[5]))
#else
		CharPtr = ((Char - newCharTable6x8[4]) * chWidth) + newCharTable6x8[0];
		if (Char < newCharTable6x8[4] || Char > newCharTable6x8[5])
#endif
		{
			//chWidth_Tmp = chWidth;
			chWidth = 0;
		...
		else
		{
			...
			if (!terminalMode)
			{
				...
				{
#ifdef __AVR_XMEGA__
					Temp = pgm_read_byte(&newCharTable6x8[Tmp + CharPtr]);
#else
					Temp = newCharTable6x8[Tmp + CharPtr];
#endif
					if (Temp == 0)
						break;
				}
			...
			if (Cursor_X + Tmp >= window.sXMin
					&& Cursor_X < window.sXMax + Tmp
					&& Cursor_Y + chHeight >= window.sYMin
					&& Cursor_Y < window.sYMax + chHeight)
			{
				if (visible)
				{
					s32 XX = 0;
					...
					{
#ifdef __AVR_XMEGA__
						Temp = pgm_read_byte(&newCharTable6x8[XX + CharPtr]);
#else
						Temp = newCharTable6x8[XX + CharPtr];
#endif
						for (YY = 0; YY < chHeight; YY++)
						{
							if (Temp & 0x1)
							{
								LcdStruct->drawPixel(XX + Cursor_X,
										YY + Cursor_Y, foreColor.ARGB);
							}
							else
							{
								if (opaque)
									LcdStruct->drawPixel(XX + Cursor_X,
											YY + Cursor_Y, backColor.ARGB);
							}
							Temp = Temp >> 1;
						}
					}
		...
		default:
			...
			if ((Cursor_X + chWidth > window.sXMax)
					&& WordWrap == true)
			{
				Cursor_Y += chHeight;
				Cursor_X = x;
...
s32 GI::Gfx::string::getStrPropTiny()
{
	if(!this)
		return 0;
...
void GI::Gfx::string::appendF(const char *pcString, ...)
{
	if(!this)
		return;
	va_list args;
...
void GI::Gfx::string::appendF(GI::string *string, ...)
{
	if(!this)
		return;
	va_list args;
...
void GI::Gfx::string::appendF(GI::Gfx::string *string, ...)
{
	if(!this)
		return;
	va_list args;
...
void GI::Gfx::string::setText(const char *pcString, ...)
{
	if(!this)
		return;
	clear();
...
#endif /* (defined(SCREEN_CONFIG_1) && defined(USED_SCREEN_CONTROLLER_1) && (SCREENS_COUNT > 0)) */
