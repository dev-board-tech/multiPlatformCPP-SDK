...
void GI::Screen::Gfx::ProgressBar::paint(void *pDisplay, s32 x_start, s32 y_start, s32 x_len, s32 y_len, tControlCommandData* control_comand)
{
	u32 color = 0;
	...
	s32 Position = (s32)GUtil::Converters::toPercent((s32)MinimValue, (s32)MaximValue, (s32)(Size.X - 8), (s32)Value);
	...
	if(Caption || Caption->length)
	{
		...
		}else
		{
			...
			u8 CharHeight = Caption->getFontHeight();
...
void GI::Screen::Gfx::ProgressBar::idle(tControlCommandData* control_comand)
{
	...
	//s32 Position = to_percentage(settings->MinimValue, settings->MaximValue, settings->Size.X - 4, settings->Value);
	//s32 OldPosition = to_percentage(settings->MinimValue, settings->MaximValue, settings->Size.X - 4, settings->Internals.OldValue);
...
GI::Screen::Gfx::ProgressBar::ProgressBar(GI::Display::Display *ScreenDisplay)
{
    if(!this)
		return;
    if(!ScreenDisplay)
