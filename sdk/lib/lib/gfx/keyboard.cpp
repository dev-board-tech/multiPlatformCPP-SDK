...
//#######################################################################################
#ifdef __AVR_XMEGA__
const u32 kbd_qwerty_keys_little_return[]  PROGMEM =
#else
static const u32 kbd_qwerty_keys_little_return[] =
#endif
{
...
//#######################################################################################
#ifdef __AVR_XMEGA__
const u32 kbd_qwerty_keys_big_return[]  PROGMEM =
#else
static const u32 kbd_qwerty_keys_big_return[] =
#endif
{
		kbd_qwerty_ctrl,'Q','W','E','R','T','Y','U','I','O','P'            ,';'          ,8               ,
		'#'           ,'A','S','D','F','G','H','J','K','L',':'            ,kbd_qwerty_up,13              ,
...
//#######################################################################################
#ifdef __AVR_XMEGA__
const u32 kbd_qwerty_keys_numeric_return[]  PROGMEM =
#else
static const u32 kbd_qwerty_keys_numeric_return[] =
#endif
{
...
//#######################################################################################
#ifdef __AVR_XMEGA__
const u8 kbd_qwerty_keys_little[][6]  PROGMEM =
#else
static const u8 kbd_qwerty_keys_little[][6] =
#endif
{
...
//#######################################################################################
#ifdef __AVR_XMEGA__
const u8 kbd_qwerty_keys_big[][6]  PROGMEM =
#else
static const u8 kbd_qwerty_keys_big[][6] =
#endif
{
		"Ctrl","Q","W","E","R","T","Y","U","I" ,"O","P"  ,";" ,"BkS",
		"Nr"  ,"A","S","D","F","G","H","J","K" ,"L",":"  ,"Up","Ent",
...
//#######################################################################################
#ifdef __AVR_XMEGA__
const u8 kbd_qwerty_keys_numeric[][6]  PROGMEM =
#else
static const u8 kbd_qwerty_keys_numeric[][6] =
#endif
{
...
//#######################################################################################
void GI::Screen::Gfx::KbdQwerty::paint(void *pDisplay, s32 x_start, s32 y_start, s32 x_len, s32 y_len, tControlCommandData* control_comand, bool refrash)
{
	u32 color = 0;
	...
	if(refrash)
	{
		...
		s32 key_size_x = ((Size.X - 4 - (kbd_border_size << 1))/13) - key_space_size;
		s32 key_size_y = ((Size.Y - 4 - (kbd_border_size << 1))/3) - key_space_size;

		s32 KeyboardLocationX = ((/*settings->Internals.Position.X + */kbd_border_size) + ((Internals.Size.X>>1) - kbd_border_size)) - (((key_size_x + key_space_size) * 13)>>1);
		s32 KeyLocationY =      ((/*settings->Internals.Position.Y + */kbd_border_size) + ((Internals.Size.Y>>1) - kbd_border_size)) - (((key_size_y + key_space_size) *  3)>>1);
		s32 KeyLocationX = KeyboardLocationX + 2;
	...
	else
	{
		...
		for(; CntInitKeys < sizeof(Internals.Keys)/sizeof(Internals.Keys[0]); CntInitKeys++)
		{
			...
			else if(KeyTmpPtr->Events.CursorUp == true)
			{
				...
				KeyTmpPtr->Events.CursorUp = false;
				if(Internals.keboardType == Kbd_Qwerty_Numeric)
					Return = (u32)kbd_qwerty_keys_numeric_return[CntInitKeys];
				else if(Internals.keboardType == Kbd_Qwerty_BigLetters)
					Return = (u32)kbd_qwerty_keys_big_return[CntInitKeys];
				else if(Internals.keboardType == Kbd_Qwerty_LittleLetters)
					Return = (u32)kbd_qwerty_keys_little_return[CntInitKeys];
...
//#######################################################################################
GI::Screen::Gfx::KbdQwerty::KbdQwerty(GI::Display::Display *ScreenDisplay)
{
    if(!this)
		return;
    if(!ScreenDisplay)
