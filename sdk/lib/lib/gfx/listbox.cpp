...
bool GI::Screen::Gfx::ListBox::paint(listboxItem* settings, void *pDisplay, s32 x_start, s32 y_start, tControlCommandData* control_comand, bool Pushed, bool Paint)
{
	...
	if(Paint)
	{
		...
		if(settings->Caption || settings->Caption->length)
		{
			...
			s32 x_str_location = x_start + 4;
			s32 y_str_location = y_start + 4;
...
void GI::Screen::Gfx::ListBox::idle(tControlCommandData* control_comand)
{
	...
	if(Internals.Control.Initiated == false)
	{
		...
		if(parentWindowHandler)
			...
			Internals.ScrollBar = new GI::Screen::Gfx::ScrollBar(Internals.pDisplay);
		...
		{
			ScrollBar->Internals.PositionOffset.X = Internals.Position.X;
			ScrollBar->Internals.PositionOffset.Y = Internals.Position.Y;
		}
							...
							Visible == true)
	{
		...
		{
			Internals.ScrollBar->Internals.PositionOffset.X = Internals.Position.X;
			Internals.ScrollBar->Internals.PositionOffset.Y = Internals.Position.Y;
		}
		...
		LcdStruct->sClipRegion.sYMax = Y_StartBox + Y_LenBox;
		GI::Screen::Util::clipLimit(&LcdStruct->sClipRegion, &back_up_clip);
		u32 color = controls_color.Control_Color_Enabled_Border_Pull;
		bool parentWindowOldStateEnabled = true;
		if(parentWindowHandler)
			parentWindowOldStateEnabled = parentWindowHandler->Internals.OldStateEnabled;
		if((!Enabled || !parentWindowOldStateEnabled) && Internals.Control.Initiated == true) {
		...
		LcdStruct->sClipRegion = back_up_clip;
			Internals.parentWindowStateEnabled = parentWindowHandler ? parentWindowHandler->Internals.OldStateEnabled : true;
		Internals.OldStateVisible = Visible;
		Internals.OldStateEnabled = Enabled;
								...
								Internals.ItemStartOnBox != Internals.OldItemStartOnBox)
	{
		...
		LcdStruct->sClipRegion.sYMax = Y_StartBox + Y_LenBox;
		GI::Screen::Util::clipLimit(&LcdStruct->sClipRegion, &back_up_clip);
		u32 EndDisplayedItems = ((Size.Y - 4) / Size.ItemSizeY) + Internals.ItemStartOnBox;
		for(CntDisplayItems = Internals.ItemStartOnBox; CntDisplayItems < EndDisplayedItems; CntDisplayItems++)
		{
			...
			else
			{
				...
				{
					u32 _CntDisplayItems = CntDisplayItems;
...
void GI::Screen::Gfx::ListBox::idle()
{
	if(!this)
	return;
...
//#######################################################################################
bool GI::Screen::Gfx::ListBox::insert(char* text, u32 location)
{
	if(!this)
		return false;
	...
	GI::Screen::Gfx::String *tmp_text = new GI::Screen::Gfx::String(Internals.pDisplay, text);
	...
	u32 TmpCntItemsToMove = ItemsCount;
...
//#######################################################################################
bool GI::Screen::Gfx::ListBox::remove(u32 location)
{
	if(!this)
		return false;
	...
	if(ItemsCount - 1 != 0)
	{
		u32 TmpCntItemsToMove = location +1;
...
GI::Screen::Gfx::ListBox::ListBox(void *parentWindowHandler)
{
	...
	GI::Screen::Gfx::String *tmp_txt = new GI::Screen::Gfx::String(Internals.pDisplay);
...
GI::Screen::Gfx::ListBox::ListBox(GI::Display::Display *ScreenDisplay)
{
    if(!this)
		return;
	if(!ScreenDisplay)
