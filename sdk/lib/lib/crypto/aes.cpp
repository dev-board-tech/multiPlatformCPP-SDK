...
void Aes::AddRoundKey(unsigned char round)
{
	unsigned char i,j;
	for(i=0;i<4;++i)
	{
...
static unsigned char Aes::Multiply(unsigned char x, unsigned char y)
{
	return (((y & 1) * x) ^
	((y>>1 & 1) * xtime(x)) ^
	((y>>2 & 1) * xtime(xtime(x))) ^
...
void Aes::BlockCopy(unsigned char* output, unsigned char* input)
{
	unsigned char i;
	for (i=0;i<KEYLEN;++i)
	{
...
void Aes::AES128_ECB_encrypt(unsigned char* input, unsigned char* key, unsigned char* output)
{
	if(!this)
		return;
...
void Aes::AES128_ECB_decrypt(unsigned char* input, unsigned char* key, unsigned char *output)
{
	if(!this)
		return;
	// Copy input to output, and work in-memory on output
...
void Aes::XorWithIv(unsigned char* buf)
{
	unsigned char i;
	for(i = 0; i < KEYLEN; ++i)
	{
...
void Aes::AES128_CBC_encrypt_buffer(unsigned char* output, unsigned char* input, unsigned int length, unsigned char* key, unsigned char* iv)
{
	if(!this)
		return;
	u32 i;
...
void Aes::AES128_CBC_decrypt_buffer(unsigned char* output, unsigned char* input, unsigned int length, unsigned char* key, unsigned char* iv)
{
	if(!this)
		return;
	u32 i;
