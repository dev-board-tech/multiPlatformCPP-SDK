...
GI::Device::Sensor::LeptonFLIR::LeptonFLIR(char *spiPath, char *i2cPath) {
...
GI::Device::Sensor::LeptonFLIR::LeptonFLIR(char *spiPath, char *i2cPath, GI::Device::Sensor::LeptonFLIR::medianRef_s medianRef)
{
	SPI = new GI::IO((char *)spiPath);
	if(!SPI->devHandler || !SPI->ioDevType)
	{
...
GI::Device::Sensor::LeptonFLIR::~LeptonFLIR() {
	if(!this)
		return;
	if(SPI)
		delete SPI;
...
SysErr GI::Device::Sensor::LeptonFLIR::write(u8 *data, u32 len)
{
	if(!this)
...
SysErr GI::Device::Sensor::LeptonFLIR::read(u8 *data, u32 len)
{
	if(!this)
		return SYS_ERR_INVALID_HANDLER;
	u32 slaveAddr = LEPTON_FLIR_ADDR;
...
SysErr GI::Device::Sensor::LeptonFLIR::writeRead(u8 *dataSend, u32 lenSend, u8 *dataReceive, u32 lenReceive)
{
	if(!this)
		return SYS_ERR_INVALID_HANDLER;
	u32 slaveAddr = LEPTON_FLIR_ADDR;
...
bool GI::Device::Sensor::LeptonFLIR::runCommand(u16 commandID) {
	if(!this)
		return false;
	if(!I2C)
	{
...
bool GI::Device::Sensor::LeptonFLIR::regWrite(u16 commandID, u16 *attributePtr, u16 attributeWordLength) {
	if(!this)
		return false;
	if(!I2C)
	{
...
bool GI::Device::Sensor::LeptonFLIR::regRead(u16 commandID, u16 *attributePtr, u16 attributeWordLength) {
	if(!this)
		return false;
	if(!I2C)
	{
...
bool GI::Device::Sensor::LeptonFLIR::directWriteBuff(u16 *attributePtr, u16 attributeWordLength) {
	if(!this)
		return false;
	if(!I2C)
	{
...
bool GI::Device::Sensor::LeptonFLIR::directWriteReg(u16 regAddress, u16 regValue) {
	if(!this)
		return false;
	if(!I2C)
	{
...
bool GI::Device::Sensor::LeptonFLIR::getStartFrame(u16 *lineBuff)
{
	if(!this)
		return false;
...
bool GI::Device::Sensor::LeptonFLIR::getLine(u16 *lineBuff)
{
	if(!this)
		return false;
	u8 line_buff[(LEPTON_FLIR_LINE_SIZE * 2) + 4];
...
bool GI::Device::Sensor::LeptonFLIR::getImage(u16 *image) {
	if(!this)
		return false;
	if(!SPI || !image)
	{
...
bool GI::Device::Sensor::LeptonFLIR::getImageBW(Color::ARGB *image, u8 alpha)
{
	if(!this)
		return false;
	if(!SPI || !image)
...
bool GI::Device::Sensor::LeptonFLIR::getImageARGB(Color::ARGB *image, u8 alpha)
{
	if(!this)
		return false;
	if(!SPI || !image)
