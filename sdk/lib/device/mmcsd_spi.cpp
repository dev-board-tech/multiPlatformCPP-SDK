...
//#######################################################################################
unsigned char GI::Device::Storage::MmcSd_Spi::ioData(unsigned char value)
{
...
//#######################################################################################
void GI::Device::Storage::MmcSd_Spi::csAssert()
{
    spiUnit->disableCsHandle = true;
    spiUnit->assert();
}
//#######################################################################################
void GI::Device::Storage::MmcSd_Spi::csDeassert()
{
    spiUnit->deassert();
}
//#######################################################################################
...
/* Wait for card ready                                                   */
/*-----------------------------------------------------------------------*/
int GI::Device::Storage::MmcSd_Spi::waitReady (    /* 1:Ready, 0:Timeout */
...
/*-----------------------------------------------------------------------*/
void GI::Device::Storage::MmcSd_Spi::deselect ()
{
    csDeassert();      /* CS = H */
...
/*-----------------------------------------------------------------------*/
int GI::Device::Storage::MmcSd_Spi::select()    /* 1:OK, 0:Timeout */
{
    csAssert();
...
/*-----------------------------------------------------------------------*/
unsigned char GI::Device::Storage::MmcSd_Spi::sendCmd (     /* Return value: R1 resp (bit7==1:Failed to send) */
...
//#######################################################################################
bool GI::Device::Storage::MmcSd_Spi::initCard()
{
...
//#######################################################################################
bool GI::Device::Storage::MmcSd_Spi::rcvrDatablock(unsigned char *buff, unsigned int bytes_to_read)
{
    unsigned char token;
...
//#######################################################################################
unsigned int GI::Device::Storage::MmcSd_Spi::readPage(void* _Buffer, unsigned long block, unsigned int nblks)
{
    if (/*drv || */!nblks)
...
//#######################################################################################
unsigned int GI::Device::Storage::MmcSd_Spi::mmcSdSpiRead(void *handler, void* _Buffer, unsigned long _block, unsigned int nblks)
{
	if(!handler)
		return false;
    GI::Device::Storage::MmcSd_Spi *intHandler = (GI::Device::Storage::MmcSd_Spi *)handler;
    if(intHandler->led)
    	intHandler->led->setOut(true);
...
//#######################################################################################
/* Send multiple byte */
void GI::Device::Storage::MmcSd_Spi::spiMulti (
...
int GI::Device::Storage::MmcSd_Spi::xmitDatablock (    /* 1:OK, 0:Failed */
...
unsigned int GI::Device::Storage::MmcSd_Spi::writePage(void* _Buffer, unsigned long block, unsigned int nblks)
{
    if (/*drv || */!nblks)
        return false;       /* Check parameter */
    if (SD_Init_OK == false)
...
//#######################################################################################
unsigned int GI::Device::Storage::MmcSd_Spi::mmcSdSpiWrite(void *handler, void* _Buffer, unsigned long _block, unsigned int nblks)
{
	if(!handler)
		return false;
    GI::Device::Storage::MmcSd_Spi *intHandler = (GI::Device::Storage::MmcSd_Spi *)handler;
...
//#######################################################################################
void GI::Device::Storage::MmcSd_Spi::mmcSdSpiIoctl(void *handler, unsigned int  command,  unsigned int *buffer)
{
	if(!handler)
		return;
    GI::Device::Storage::MmcSd_Spi *intHandler = (GI::Device::Storage::MmcSd_Spi *)handler;
...
//#######################################################################################
void GI::Device::Storage::MmcSd_Spi::mmcSdSpiIdle(void *handler)
{
	if(!handler)
		return;
    GI::Device::Storage::MmcSd_Spi *intHandler = (GI::Device::Storage::MmcSd_Spi *)handler;
...
//#######################################################################################
void GI::Device::Storage::MmcSd_Spi::mmcSdGetRawCid(void *handler, unsigned long *rawCid)
{
	if(!handler)
		return;
	GI::Device::Storage::MmcSd_Spi *intHandler = (GI::Device::Storage::MmcSd_Spi *)handler;
...
void GI::Device::Storage::MmcSd_Spi::mmcSdGetRawCsd(void *handler, unsigned long *rawCsd)
{
	if(!handler)
		return;
	GI::Device::Storage::MmcSd_Spi *intHandler = (GI::Device::Storage::MmcSd_Spi *)handler;
...
//#######################################################################################
GI::Device::Storage::MmcSd_Spi::MmcSd_Spi(unsigned int unitNr, char *spiPath, char *cdPath, char *ledPath)
{
    GI::Dev::DevRequest::request(spiPath, &spiUnit);
    GI::Dev::DevRequest::request(ledPath, &led);
    GI::Dev::DevRequest::request(cdPath, &sdPresent);
    idle_Ptr = &GI::Device::Storage::MmcSd_Spi::mmcSdSpiIdle;
    read_Ptr = &GI::Device::Storage::MmcSd_Spi::mmcSdSpiRead;
    write_Ptr = &GI::Device::Storage::MmcSd_Spi::mmcSdSpiWrite;
    ioctl_Ptr = &GI::Device::Storage::MmcSd_Spi::mmcSdSpiIoctl;
    getRawCid_Ptr = &GI::Device::Storage::MmcSd_Spi::mmcSdGetRawCid;
    getRawCsd_Ptr = &GI::Device::Storage::MmcSd_Spi::mmcSdGetRawCsd;
    this->unitNr = unitNr;
...
GI::Device::Storage::MmcSd_Spi::~MmcSd_Spi()
{
