...
class Mpr121 {
public:
	Mpr121(char *i2cPath, char *irqPath, unsigned char icNr);
	~Mpr121();
	SysErr idle(mpr121_ret_t *return_keys);
private:
	SysErr readShort(unsigned char reg, unsigned short *return_data);
	SysErr read(unsigned char reg, unsigned char *return_data);
