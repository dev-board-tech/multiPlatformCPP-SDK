		...
		uint8_t interface)
{
	...
	if (interface < phost->device.CfgDesc.bNumInterfaces)
	{
		...
		USBH_UsrLog ("Switching to Interface (#%d)", interface); USBH_UsrLog ("Class    : %xh", phost->device.CfgDesc.Itf_Desc[interface].bInterfaceClass ); USBH_UsrLog ("SubClass : %xh", phost->device.CfgDesc.Itf_Desc[interface].bInterfaceSubClass ); USBH_UsrLog ("Protocol : %xh", phost->device.CfgDesc.Itf_Desc[interface].bInterfaceProtocol );
	}
	else
	{
...
static USBH_StatusTypeDef USBH_HandleEnum(USBH_HandleTypeDef *phost)
{
	...
	case ENUM_GET_FULL_DEV_DESC:
		...
		{
			USBH_UsrLog("PID: %xh", phost->device.DevDesc.idProduct ); USBH_UsrLog("VID: %xh", phost->device.DevDesc.idVendor );
