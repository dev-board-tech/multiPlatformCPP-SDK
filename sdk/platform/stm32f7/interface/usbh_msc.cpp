...
typedef enum
{
	APPLICATION_IDLE = 0, APPLICATION_START, APPLICATION_RUNNING,
...
static DRESULT USBH_read(BYTE lun, BYTE *buff, DWORD sector, UINT count)
{
	...
	if ((DWORD) buff & 3) /* DMA Alignment issue, do single up to aligned buffer */
	{
		...
		{
			status = USBH_MSC_Read(&usb_msc_host_param[lun], lun,
					sector + count, (unsigned char *) scratch, 1);
			if (status == USBH_OK)
			{
	...
	{
		status = USBH_MSC_Read(&usb_msc_host_param[lun], lun, sector, buff,
				count);
	}
...
static DRESULT USBH_write(BYTE lun, const BYTE *buff, DWORD sector, UINT count)
{
	...
	if ((DWORD) buff & 3) /* DMA Alignment issue, do single up to aligned buffer */
	{
		...
		while (count--)
		{
			...
			status = USBH_MSC_Write(&usb_msc_host_param[lun], lun,
					sector + count, (BYTE *) scratch, 1);
			if (status == USBH_FAIL)
			{
				break;
			}
	...
	{
		status = USBH_MSC_Write(&usb_msc_host_param[lun], lun, sector,
				(BYTE *) buff, count);
	}
...
unsigned int GI::Dev::UsbHMsc::read(void *heandler, void *ptr, unsigned long block,
		unsigned int nblks)
{
	...
	unsigned char lun = 0;
	if (LedStatusUsb[lun])
		LedStatusUsb[lun]->setOut(1);
	if (USBH_read(lun, (BYTE *) ptr, block, nblks) == RES_OK)
	{
		if (LedStatusUsb[lun])
			LedStatusUsb[lun]->setOut(0);
...
unsigned int GI::Dev::UsbHMsc::write(void *heandler, void *ptr, unsigned long block,
		unsigned int nblks)
{
	//unsigned int status = CPUIntStatus();
	//CPUirqe();
	...
	if (LedStatusUsb[lun])
		LedStatusUsb[lun]->setOut(1);
	if (USBH_write(lun, (const BYTE *) ptr, block, nblks) == RES_OK)
	{
		if (LedStatusUsb[lun])
			LedStatusUsb[lun]->setOut(0);
		//if(status & 0x80) CPUirqd();
...
void GI::Dev::UsbHMsc::ioctl(void *heandler, unsigned int command,
		unsigned int *buffer)
{
	USBH_HandleTypeDef * _ctrl = (USBH_HandleTypeDef *)heandler;//(HCD_HandleTypeDef *)ctrl;
	//DRESULT res = RES_ERROR;
	MSC_LUNTypeDef info;
	unsigned char lun = 0;
	switch (command)
	...
	case GET_SECTOR_COUNT:
		if (USBH_MSC_GetLUNInfo(_ctrl, lun, &info)
				== USBH_OK)
		{
...
GI::Dev::UsbHMsc::UsbHMsc(unsigned int instance, GI::Dev::Gpio* StatusLed)
{
	LedStatusUsb[instance] = StatusLed;
	//if(FATFS_LinkDriver(&USBH_Driver, USBDISKPath) == 0)
	//{    /*##-2- Init Host Library ################################################*/
	USBH_Init(&usb_msc_host_param[instance], USBH_UserProcess, instance);
...
void GI::Dev::UsbHMsc::idle(unsigned int instance)
{
	...
	case APPLICATION_START:
...
#endif
		{
			...
			if (g_sDirObject.fopendir(g_cCwdBuf1) == FR_OK)
			{
				...
				uart.printf(DebugCom, "USBH %d ERROR oppening path\n\r", 0);
#endif
		}
#ifdef USBH_MSC_DEBUG_EN
		else if (DebugCom)
			uart.printf(DebugCom, "USBH %d ERROR mounting disk\n\r", 0);
#endif
		Appli_state = APPLICATION_IDLE;
		break;
