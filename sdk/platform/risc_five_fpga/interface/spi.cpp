...
volatile FPGA_SPI_t* SPI_BASE_PTRS[] =
{
#ifdef FPGA_SPIA
		&FPGA_SPIA,
#else
...
#endif
	CfgSpi *int_cfg = (CfgSpi *)cfg->cfg;
	FPGA_PORT_t *BaseAddr = GPIO_BASE_PTRS[int_cfg->cs >> 5];
	BaseAddr->OUTCLR = (1 << (int_cfg->cs % 32));
	err = SYS_ERR_OK;
	return SYS_ERR_OK;
}
/*#####################################################*/
...
int GI::Dev::Spi::deassert()
{
	...
	CfgSpi *int_cfg = (CfgSpi *)cfg->cfg;
	FPGA_PORT_t *BaseAddr = GPIO_BASE_PTRS[int_cfg->cs >> 5];
	BaseAddr->OUTSET = (1 << (int_cfg->cs % 32));
#if (USE_DRIVER_SEMAPHORE == true)
		...
		unsigned char *buffRead, unsigned int lenRead)
{
	...
	for(; cnt < lenWrite; cnt++)
	{
		...
		while(!(port->STATUS & FPGA_SPI_IF_bm)) { }
		//tmp_read = port->DATA;
	}
