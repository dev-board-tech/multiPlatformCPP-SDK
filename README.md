...
In future this project will be extended to AVR32, AM335x, STM32F0, STM32F1, STM32F3 and other platforms.
...
At this moment include next features:
	...
	STM32F4 (System Workbench for STM32 IDE):
 	...
 	STM32F7 (System Workbench for STM32 IDE):
		Board:
			STM32F769I-DISCO configuration files.
		Drivers:
			I2C.
			UART.
			SPI.
 	...
 	CC13xx (Code Compose Studio):
		Board:
			...
			CC1310LauchPad configuration files.
		Drivers:
			I2C(NOT TESTED).
			UART.
			SPI.
 	...
 	ATxmega A,B... (Atmel Studio):
		Board:
			mikroXmega from MikroElektronika.
		Drivers:
			I2C(NOT TESTED).
			UART.
			SPI.
	...
	ATxmega FPGA (Atmel Studio & XILINX Vivado)
		Board:
			NEXIS VIDEO from DIGILENT.
		Drivers:
			RTC IP.
			UART IP.
			SPI IP.
...
* AK8975 compass sensor driver using I2C interface.
* BMA250 accelerometer sensor driver using I2C/SPI interface
* BMP180 pressure, altimeter and temperature sensor driver using I2C interface.
* FT5x06 capacitive touch screen driver using I2C interface
* HIH613x humidity and temperature sensor driver using I2C interface.
* ILI9341 TFT display controller driver using SPI interface.
* L3GD20 gyroscope sensor driver using SPI interface.
* Lepton FLIR sensor driver using SPI interface.
* LPS25hb pressure and teperature sensor driver using I2C/SPI interface.
* LSM9ds accelerometer, gyroscope, compass and temperature sensor driver using I2C/SPI interface.
* MI0283 color LCD display controller driver using GPIO interface.
* MMCSD driver using SPI interface.
* MPL3115a2 pressure altimeter and temperature sensor driver using I2C interface.
* MPR121 capacitive touch controller driver using I2C interface.
* MPU60x0 and MPU9x50 accelerometer, giroscope and temperature sensor driver using I2C interface.
* SSD1306 OLED display driver using SPI interface.
* SSD1331 96x64 RGB OLED display driver using SPI interface.
* ST7565 LCD STN display driver using SPI interface.
* STMPE811 resistive touchscreen controller driver using I2C interface.
* TCN75s temperature sensor driver using I2C/SPI interface.
...
int main(void)
{
	...
	while(1)
	{
		...
		if(timer_touch.tick())
		{
			 ...
			 *	returning true if the "OK" button has been hit or false if no.
			 */
			if(pass->idle())
			{
				/*
